			...
			@include ('layouts.header')
				...
				<form action="{{ route ('guardarProductos')}}" method="POST" name="nuevo" enctype="multipart/form-data">
					...
					<div style="padding: 1%;">
						...
						<td><input type="text" name="nombre_producto" id="nombre_producto" value="{{ old('nombre_producto')}}"></td>
					</div>
					@if($errors->first('nombre_producto')) <i>{{$errors -> first ('nombre_producto')}}</i>@endif

					<div style="padding: 1%;">
						Numero de existencias : <input type="text" name="no_existencias" id="no_existencias" value="{{ old('no_existencias')}}">
					</div>
